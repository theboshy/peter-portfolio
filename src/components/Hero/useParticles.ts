...
export const useParticles = () => {
  ...
  const createParticle = useCallback((x: number, y: number, color: string, duration: number) => {
  ...
  const handleScrollToContact = useCallback(async (e: React.MouseEvent<HTMLAnchorElement>) => {
    e.preventDefault();
    ...
    for (let i = 0; i < 6; i++) {
      ...
      setTimeout(() => {
        createParticle(x, y, i % 2 === 0 ? '#238636' : '#30363D', duration - (i * 50));
      }, i * 50);
    }
