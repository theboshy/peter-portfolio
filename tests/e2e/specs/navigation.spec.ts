...
test.describe('Navigation', () => {
  ...
  test('should handle language switching', async () => {
    ...
    await expect(homePage.getInTouchButton).toHaveText(languages.spanish.getInTouch);

    // Switch back to English
    await homePage.navigation.changeLanguage(languages.english.label);
    await expect(homePage.getInTouchButton).toHaveText(languages.english.getInTouch);
  });
